...
const ProductsTable = () => {
  ...
  const handleDelete = async (productId) => {
    try {
      ...
      await axios.delete(`http://localhost:5454/api/admin/products/${productId}/delete`, config);
        ...
        {!!results && results.map((result, index) => (
          <React.Fragment key={index}>
            <div className="listproduct-format-main listproduct-format">
              <img src={result.imageUrl} alt="" className="listproduct-product-icon" />
              <p>{result.brand}</p>
              <p>{result.title}</p>
              <p>{result.price}</p>
              <p>{result.quantity}</p>
              <img onClick={() => handleDelete(result.id)} src={cross_icon} alt="" className="listproduct-remove-icon" />
